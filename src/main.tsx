...
const Action = lazy(() => import("./pages/Action.tsx"));
...
function RouteLoading() {
  return (
    <div className="min-h-screen flex items-center justify-center bg-background">
      <div className="animate-pulse text-muted-foreground font-condensed text-sm tracking-[0.15em] uppercase">
        Loading...
      </div>
    </div>
  );
...
// Preloader state — only shows on initial load
let preloaderDismissed = false;

function PreloaderWrapper({ children }: { children: React.ReactNode }) {
  const [showPreloader, setShowPreloader] = useState(!preloaderDismissed);
